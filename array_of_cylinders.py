...
parser = argparse.ArgumentParser(
                    prog='array_of_cylinders',
                    description='''One rigid particle flows in Poiseuille. ''',
...
def save_pic(time, u, rho, particles, folder, ny):
...
def save_velocity(time, u, particles, folder):
    for i, particle in enumerate(particles):
        ...
        y = particle[-1, 1]
        weight = y % 1
        unorm = (1 - weight) * unorm[:, int(y)] + weight * unorm[:, int(y) + 1]
        data += list(unorm)
...
if __name__ == '__main__':
    ...
    folder = 'tmp_cs2_%.2f_re_%.2f_nu_%.2f_u_%.2f_tau_%.2f_cy_%.2f_W_%d/'%(cs2, Re, nu, \
                                                              uw, tau, cy, D)
    ...
    for i in range(num_particles):
        rad = 10
        lst_rad.append(rad)
        N =  int(1.3 * (2 * rad) * np.pi * 1.)
        lst_N.append(N)
        length_particle = 2 * np.pi * rad 
        lst_length.append(length_particle)
        lst_delta_v.append(length_particle / N)
        lst_mass.append(np.pi * rad ** 2 * rho_particle)
        particles.append(make_particle(2 * (Ds / 2) + 12 * (Ds / 2) * i, cy, 
                                        uw, ny, N, L, rad, vel_poiseuille=False))
    ...
    for time in range(0, maxIter):
        ...
        for p in range(num_particles):
            ...
            #for pp in range(p + 1, num_particles):
        ...
        if (time % fig_out == 0):
            ...
            save_pic(time, u, rho, particles, folder, ny)
